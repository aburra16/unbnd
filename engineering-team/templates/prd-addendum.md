...
**Reconciles:** the source PRD *(immutable — never edited)* — one of `unbnd-prd.md`, `engineering-team/phase2-prd.md`, or `product-team/prd/<slug>.md`.
**Build audit:** `engineering-team/audits/<book-slug>/audit.md`
**Date:** <DATE>
**Authored by:** engineering (Reviewer at book scope)

> This addendum stands **beside** the original PRD; it never edits it. It records where the built product diverged from the plan, why, and what the next product cycle should pick up. The product team reads this when scoping the next phase, then issues a superseding PRD. Engineering does not write into `product-team/` — this is the read-across-the-boundary return edge, mirroring how engineering reads the product team's queue.
...
- **Domain model:** <DList shapes / entities that changed shape in practice>
- **Design rules:** <copy or visual rules that proved wrong or incomplete>
