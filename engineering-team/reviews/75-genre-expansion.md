...
### Non-blocking
1. **Substring matching false-positives (`subjects → genres`).** The rule table uses `includes()`, so a subject can match a genre it shouldn't: `"conscience"` → `science`, `"war crimes"` → `mystery`, etc. (confirmed). This is the precision/recall tradeoff the ADR accepts (genre is a revisable assertion — a curator corrects it), and these subjects are low-frequency, but the class is real. *Suggestion: word/token-boundary matching (or a small denylist for the known traps) in a follow-up; not worth blocking the expansion.*
