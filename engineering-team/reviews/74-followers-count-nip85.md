...
### Non-blocking
1. **Duplication between `weights()` and `followers()` in `brainstorm.ts` — RESOLVED in review.** The two methods shared ~30 lines (cache check → `#serviceKey` → relay union → `query` → per-event parse/union). Extracted a private `#readAssertionUnion(observer, targets, cache, pick)` helper; `weights()` and `followers()` are now thin wrappers differing only in their per-event `pick` (rank→weight vs followers→count). Trust suite stays 40/40, `pnpm -r test` green — behavior unchanged.
