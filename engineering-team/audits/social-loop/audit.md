...
- [x] **Deploy + ops sequence — DONE 2026-06-11** (post-close): pushed; staging runs the full book. Recast: 11,282 books → 20,952 assertions, all 16 genres non-empty (none dropped). Index rebuilt, shelves refreshed, promoter cycle run, `PUBLIC_ORIGIN` verified via a live unfurl card. The documented worker crontab was found NEVER INSTALLED and is now live (`install-cron`; promoter+sync 5min, shelves hourly, nightly index). The CI run also exposed and fixed two latent DB defects (the promotions CHECK constraint rejecting demote statuses; drizzle-wrapped 23505 detection) and 4 stale visual baselines (refreshed via the documented intentional path). §10 is now fully measurable. ~~push `main` (87 commits, #71–#82 + close)~~ → CI deploys staging → set `PUBLIC_ORIGIN` → run `seed:recast` (check yield; drop empty genres) → calibrate + set `AUTO_PROMOTE_CURATOR_COUNT` → confirm promoter cron → **operator: age-encrypt `LIBRARIAN_NSEC`** (pending) → verify §10 metrics.
