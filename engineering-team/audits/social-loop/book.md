...
**Block 3 status (2026-06-09): ALL SIX MERGED** (#77–#82, each through the full five-gate cycle, all reviews PASS). The build phase of this book is complete; book-close is next.
...
- **#82 operator ops task (PENDING, relay to the operator):** `age`-encrypt `LIBRARIAN_NSEC` at rest on the droplet (it currently sits plaintext in the promoter's env file). Cannot be done from the repo; track until the operator confirms.
...
- **Review #82 finding 1:** a `demote_failed` row still falls through `PromoteCell` to the Promote button (pressing it answers `already`; only `enqueueDemotion` retries a `demote_failed`). Rare worker-failure state, same one-tick honesty class as the (now fixed) demote_pending case. Pair it with the promoter `failed`-retry gap above if a retry-policy story happens.
