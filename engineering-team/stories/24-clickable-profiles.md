...
- **Q3 (reviews-block byline → link, confirm):** today the review byline (`review-name`) is plain text. Confirm making the existing review byline itself the link (consistent with rate-only entries) is in scope here, so reviewers and rate-only raters are both reachable. PO recommends: yes.
