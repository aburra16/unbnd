...
## Background
Three entries: what self-ownership means in practice, the sovereignty deep entry behind the Settings card ("key" introduced as the key to your account, what it does before any property of it), and the one clearly marked wider-network note, the guide's single E-wall exemption and the first live use of the `taxonomy-exempt` mechanism. Process deliverables ride with it: the staying-current rule lands in the engineering definition of done (the review checklist), and the scope inventory reconciles in the book with zero gaps. A rider: the api suite gains timeout headroom after a load-induced 5s timeout at the #90 gate (a new, milder flake class than the retired transport one).
