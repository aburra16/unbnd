...
3. **Anchor in the PRD.** Identify the PRD section that motivates the work and quote the relevant acceptance language. If the request would expand PRD scope (touches §11.3 "Out of Scope"), pause and flag it — the user must explicitly re-scope before the story can proceed.
4. **Draft the story.** Acceptance criteria must be testable from outside.
5. **Show the draft.** Iterate with the user until approved.
6. **Save** the file.
7. **Gate:** ask explicitly: "Story approved? Ready to enter Architecture?"
8. On approval, hand off to `/design-architecture`.
...
- Inventing new DList shapes when an existing Tapestry concept covers the case. The Architect names the shape; the PO just identifies that the story will touch a particular surface.
