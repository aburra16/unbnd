...
4. **Bracket the book of work (eager anchor).** A *book* is a PRD (or one roadmap phase of one), or — with no PRD — a bounded ask. Decide whether this request starts a new book or joins an open one (`engineering-team/audits/*/book.md` with `Status: Open`):
   - **Joins an open book** → add its story to that `book.md`. Nothing else to capture.
   - **Starts a new book, PRD-backed** → create `engineering-team/audits/<book-slug>/book.md` from `templates/book.md`, anchor pointing at the PRD §sections it realizes (`unbnd-prd.md`, `engineering-team/phase2-prd.md`, or `product-team/prd/<slug>.md`). Completion will be *computed*.
