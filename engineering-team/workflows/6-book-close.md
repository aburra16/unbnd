...
## Cadence
This phase does **not** run per story. It runs once, when a **book of work** completes — a PRD (or one roadmap phase of a PRD), or, with no PRD, the ask captured in the book's acceptance frame. The per-story cycle (Phases 1–5) is orthogonal and keeps running underneath.
...
## Input
- The book manifest `engineering-team/audits/<book-slug>/book.md` (anchor + story set). If none exists, reconstruct one first (see Provenance in step 1).
- The anchor: the PRD §sections, or the acceptance frame.
- All stories / ADRs / reviews for the book (in `engineering-team/stories/` + `stories/done/`, `decisions/`, `reviews/`), plus any incremental `## Deviations` logs.
- The book diff: `git diff <base>..<head>`.
...
Both are **engineering-authored and live under `engineering-team/`.** The product team *reads* them to scope the next phase — engineering never writes into `product-team/`. This mirrors the forward handoff, where engineering reads the product team's queue.
...
2. **Roll up the per-story record.** Aggregate, don't re-derive: ADR `Consequences`/`Out of scope`, story `Out of scope`/`Open questions`, review verdicts, and any Implementer `## Deviations` logs.
...
7. **Run the gates** at close: `pnpm -r typecheck && pnpm -r test` (and `pnpm --filter @unbnd/web build` if UI was in scope); record the results in the audit.
...
## Book retirement
A closed book's folder moves under `audits/done/<book-slug>/` once the next phase has ingested it (one `git mv` on the directory). Everything outside `done/` is live; everything under it is shipped and read-only by convention.
