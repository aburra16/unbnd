...
### C-5 — Profile information architecture: progressive disclosure of nostr internals — DONE (Story 29, ADR 0030, 2026-06-01)
Today `/profile/me` renders the raw `npub1…` string directly under the display name (`ProfileMe.tsx`), and the Settings page exposes nostr fields flat. For a custodial (email-signup) user who has never heard of nostr, a 63-char `npub` under their name is noise with no affordance or explanation — it reads as a bug, not an identity. Restructure the profile/settings surface with **progressive disclosure**: a clean human-facing default (avatar, name, bio, activity, shelves) with the protocol-level identity tucked behind an explicit **"Advanced" / "Nostr identity"** tab or section. That section is the right home for: the npub (with **click-to-copy** + a one-line "this is your portable identity across nostr apps" explainer) and a relay list. Sovereign (NIP-07) users may see the npub more prominently since it's already their mental model; the disclosure is primarily for custodial. **Engineering note:** purely a web IA/component refactor — no new API, no schema change; consumes the same `session.user.npub` + `useProfileMeta`. Pairs naturally with C-3 (provider→npub federation) when that lands, since the "Advanced" section is where linked-identities would surface. UX-copy rule applies (no slop; plain language for the explainer).

**Correction (2026-06-01, verified during Story 29):** an earlier draft of this item claimed the **sovereignty upgrade path (export nsec → NIP-07) "already exists from Phase 1."** That is FALSE — Phase 1 only encrypts the custodial key at rest (`encryptedNsecPassword`/`encryptedNsecBackup`, decrypted transiently for server signing); there is no nsec-export UI. Story 29 scoped nsec-export OUT and did not build it; it remains a **future story** (and earns its own design, since exporting a custodial key is a sensitive flow). What shipped in Story 29: the npub moved behind a Settings "Nostr identity" section (canonical home) + a tier-differentiated `/profile/me` header (custodial clean + "Advanced settings" link; sovereign keeps a labeled, copyable, monospace truncated npub chip), a shared `CopyButton`, on-page explainer (no tooltips). Relay-list display and nsec-export remain deferred.

### C-6 — Settings: a dedicated "Nostr" / "Advanced" tab
Story 29 put the npub behind a "Nostr identity" section in Settings and pointed the custodial profile header's **"Advanced settings"** link at `/settings`. The next step is a dedicated, standalone **Advanced/Nostr tab** in Settings that the "Advanced settings" link targets directly, consolidating the protocol-level surface in one place: the npub (click-to-copy, shipped), **relay selection/display** (deferred from C-5), and the **nsec claim/export** sovereignty-upgrade path (does NOT exist today — see the C-5 correction; exporting a custodial key is a sensitive flow that earns its own design). Web IA refactor + the nsec-export design (the one genuinely new/sensitive piece). Pairs with C-3 (provider→npub federation) — the same tab is where linked identities would surface.

### C-7 — Trust-weighted curator roles (community-anointed) [design note; implementation Phase 3]
Evolve the curator gate from Story 30's **general** house-PoV GrapeRank weight ("broadly trusted on nostr") into a **domain-specific, community-anointed role**: a `curator` tag-assertion targeting a *pubkey* (mechanically Story-25's trust-weighted tag consensus, but the target is a person, not a book). This is the project's original recursive-WoT role model, and it generalizes into a single `trust.roleScore(pubkey, role)` primitive that also powers **§2.8** (accusatory-tag write gate) and **§2.10** (verified-author / trust-tier badges) — "is this pubkey trusted *for role X*."

**Consensus model (decision of record for this note):** anyone may sign a `curator` tag on someone; a zero-house-weight asserter's tag means nothing. Curator status sticks when **≥ N asserters, each with house-PoV weight ≥ W**, have applied the tag — a legible count-gate ("N trusted people vouched"), not a continuous weighted sum. `N` and `W` are configurable knobs (placeholder: N=10, W=0.2). **Scale note:** GrapeRank weights in this codebase are **0–1** (`rank/100`), so "score above 20" = weight ≥ 0.2 (rank 20); pin the scale explicitly when built. Sybil-resistant by construction (fake endorsers carry ~0 weight). Apply/dispute polarity gives revocation; exclude self-assertion.

**Bootstrapping (breaks the chicken-and-egg):** an explicit **seed-curator allowlist** in operator config — 5–10 handpicked pubkeys that resolve as curators regardless of community consensus, OR'd with the consensus path: `isCurator = inSeedSet(pubkey) || communityConsensus(pubkey) ≥ gate`. A single founder tag can't satisfy the N-asserter count, so the seed set is a separate, **auditable**, retire-as-consensus-fills lever — not hidden magic. Story 30's general-weight `canPromote` gate remains the cold-start fallback; this layers a more precise signal on top, it does not replace Story 30.

**Session resolution + UI:** resolve the role once at session time (cache an `isCurator` flag, like `canPromote`) and serve a **curator UI** — a prominent "Curate"/Submissions surface for curators, tucked away for everyone else (today Submissions is a small footer link for all). **Depends on house-observer maturity** (the asserter weights come from the house vantage — thin on nosfabrica today), so it lands best after a real/curated house graph.

**Cheap Phase-2 intermediate (optional, available now):** the *UI-gating half* can ship today on the existing Story-30 gate — lift `canPromote` to a session-level `isCurator` flag and conditionally surface the curator nav/entry. This delivers most of the UX win without the role-assertion system; the C-7 work is the upgrade to *how* curator status is determined.
