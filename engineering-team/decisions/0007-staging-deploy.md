...
**Follow-up (post-go-live):** the initial split of `publish-images.yml` (build) and `deploy.yml` (deploy) both triggered off the same "CI succeeded on main" event and ran in parallel — a race where deploy could `docker compose pull` the *previous* `:latest` before the new images finished building. Merged into a single ordered workflow `staging.yml`: a `build` matrix (api, web) tagged with the commit SHA, then a `deploy` job (`needs: build`) that checks out and deploys that exact SHA via `UNBND_IMAGE_TAG`. Deterministic (code + images match the same commit), no race, and rollback is re-pointing to a prior SHA. The data-layer image stays a separate manual publish at `:latest`. Staging is `main`-auto; a gated `production` Environment + tag/release-triggered promotion of the same SHA image is the future prod path (out of scope here).
