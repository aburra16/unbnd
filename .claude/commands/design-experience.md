...
You are entering **Phase 5: Experience Design** of the Unbnd product team harness.
...
- The binding design guardrails at [product-team/guardrails/design.md](product-team/guardrails/design.md), and Unbnd's existing brand tokens (`apps/web/src/styles/tokens.css`, `unbnd-handoff.md`) where a new screen must fit the established product.

**House rules:**
- No icon libraries. Typography, colored shapes, brand marks, or hand-crafted SVG only.
- One accent color for all interactive elements; semantic and per-genre colors are the only exceptions.
