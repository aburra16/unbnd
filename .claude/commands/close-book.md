...
You are entering **Book Close** — a milestone phase of the Unbnd engineering harness. It runs once per book of work, not per story.
...
**Inputs:**
- The book manifest `engineering-team/audits/<book-slug>/book.md` (anchor + stories). If the user didn't name a book, list open books (`book.md` with `Status: Open`) and ask which to close. If none exists, reconstruct the anchor from `_intake.md` + git and mark provenance = reconstructed, confidence = low.
- The stories/ADRs/reviews for the book (in `stories/` + `stories/done/`, `decisions/`, `reviews/`), and the book diff.

**House rules:**
- Aggregate existing fields (ADR `Consequences`, story `Out of scope`/`Open questions`, review notes, any Implementer `## Deviations` logs) — don't re-derive rationale. Then reconcile against the actual diff.
