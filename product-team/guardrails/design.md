...
These rules apply to every design decision made during the product process and carry into the design guide the engineering team builds from. They mirror Unbnd's visual no-slop rule; the authoritative project-wide list lives in `memory/feedback_unbnd_copy_and_visual.md`.
...
- **One accent color.** The brand accent is used for all interactive elements. Semantic colors (success, warning, error) and per-genre/signal colors are the only exceptions. No decorative color variety.
...
- **Zero tech debt by design.** No "we'll fix it later" hacks. If the right solution is too expensive for MVP, defer the *feature* rather than ship a hack. (This is the quality-bar rule — see `memory/feedback_unbnd_quality_bar.md`.)
- **No hardcoded values in components.** Colors, spacing, typography, and radii come from design tokens (CSS custom properties in `tokens.css`). A component never contains a hex color or a pixel value that isn't a token reference.

## Where these are enforced
- The **Product Designer** honors them in Phase 5 and encodes them as the design guide's "Design principles."
- The **engineering team's Reviewer** checks the implementation against the design guide and the brand tokens during `/review-changes`.
