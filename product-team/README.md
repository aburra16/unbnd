...
The stories queue groups story briefs into ordered blocks. The engineering team's Product Owner reads the queue and promotes each brief into a formal `engineering-team/stories/<n>-<slug>.md` via `/plan-feature`, referencing the PRD and guides as context. (Unbnd's engineering tree is flat — one `<n>-<slug>` namespace; the queue's block grouping is a suggested sequence, not a folder structure. See `engineering-team/MIGRATION-epic-folders.md` for the unused epic-folder option.) See `Handoff` in `workflows/7-story-decomposition.md` for the exact seam.
