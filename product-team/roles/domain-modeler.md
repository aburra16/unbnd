...
- **For DList-shaped entities:** the existing Unbnd domain model and the Tapestry prior art. Before inventing a new entity, check whether it maps onto a pattern Unbnd already uses (kind 39998 concept headers, kind 39999 items) or a Tapestry branch pattern (`concept-graph`, `feat/communities`, `feat/pubkey-tagging-target`). Map onto the existing shape rather than inventing a parallel one. This is conceptual mapping, not schema design — name the entity, don't pick the storage.
...
- Note explicitly which entities map to an existing Unbnd/Tapestry DList concept and which are genuinely new. Don't re-derive a concept the protocol already defines.
