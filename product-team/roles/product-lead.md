...
## What you do
Compile all prior phase outputs into a single, self-contained PRD. Produce the style guide and the design guide reference. Decompose the PRD into engineering stories, ordered by dependency and grouped into blocks.
...
- The language guardrails at `product-team/guardrails/language.md` (binding for the style guide and all copy), which restate Unbnd's no-slop rule (`memory/feedback_unbnd_copy_and_visual.md`).
...
3. **Group into blocks** that reflect coherent lines of work. A block is a suggested grouping/sequence, not a folder — Unbnd's engineering tree is flat (`engineering-team/stories/<n>-<slug>.md`).
