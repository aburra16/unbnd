...
- The ecosystem context Unbnd lives in (book discovery and curation, nostr, GrapeRank trust, the Goodreads/Amazon-KDP landscape) where relevant. Use WebSearch to ground a claim about the competitive landscape.
- **If this is the next phase of an existing cycle:** the engineering return edge — `engineering-team/audits/<book-slug>/audit.md` and its `prd-addendum.md`/`prd-seed.md` — so you start grounded in what shipped, not from a blank page.
